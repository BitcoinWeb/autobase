...
class Linearizer {
  constructor (bitstream, output, opts = {}) {
  ...
  async _update () {
    const latestClock = await this.bitstream.latest()
    ...
    for await (const node of this.bitstream.createCausalStream()) {
      if (this.invalidated) return null
  ...
  async update () {
    ...
    while (nodes.length) {
      ...
      const inputNode = await this.bitstream._getInputNode(node.change, node.seq)
      if (this.invalidated) return null
...
module.exports = class LinearizedView extends EventEmitter {
  constructor (bitstream, opts = {}) {
    ...
    this.ready = () => this.bitstream.ready()
  }
  ...
  get writable () {
    return !!this.bitstream.localOutput
  }
  ...
  _refreshLinearizers () {
    if (this.writable) {
      if (this._linearizersByOutput.has(this.bitstream.localOutput)) return
      this._linearizersByOutput.set(this.bitstream.localOutput, new Linearizer(this.bitstream, this.bitstream.localOutput, {
        header: this._header,
        apply: this._apply,
        persist: true
      }))
    ...
    for (const output of this.bitstream.outputs) {
      ...
      this._linearizersByOutput.set(output, new Linearizer(this.bitstream, output, {
        header: this._header,
        apply: this._apply,
        persist: false
      }))
  ...
  async _update () {
    ...
    await Promise.all(this.bitstream.outputs.map(i => i.update()))
    await this._refreshLinearizers()

    let bestOutput = this.bitstream.localOutput
    if (!bestOutput) {
      for (const output of this.bitstream.outputs) {
  ...
  async append (block) {
    await this.ready()
    if (!this._updating) throw new Error('Cannot append to a CombinedUnichain outside of an update operation.')
