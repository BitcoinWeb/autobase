...
module.exports = class KeyCompressor {
  ...
  async decompress (compressedClock, seq) {
    ...
    const blocks = await Promise.all(seqs.map(seq => this.chain.get(seq)))
  ...
  async compress (clock, seq) {
    ...
    if (!this.initialized && this.chain.length > 0) {
      const headBuf = await this.chain.get(this.chain.length - 1)
      const state = { start: 0, end: headBuf.length, buffer: headBuf }
      const head = NodeSchema.decode(state)
      await this._decompress(head.clock, this.chain.length - 1)
    }
  ...
  async resolvePointer (keyPointer) {
    ...
    const block = await this.chain.get(keyPointer.seq)
